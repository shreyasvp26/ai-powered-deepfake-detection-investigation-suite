...
% ---- RGB Stream (top) ----
\node[sbox=blue!10, right=2cm of inp, yshift=1.3cm, minimum width=3cm] (enet) {EfficientNet-B4};
...
% ---- Freq Stream (bottom) ----
\node[pbox=orange!8, right=2cm of inp, yshift=-1.3cm, minimum width=1.4cm] (gray) {Grayscale};
\node[pbox=orange!8, right=0.6cm of gray, minimum width=1.2cm] (srm) {SRM};
\node[pbox=orange!8, right=0.4cm of srm, minimum width=1.2cm] (fft) {FFT};
\node[sbox=orange!10, right=0.8cm of fft, minimum width=2.4cm] (rnet) {ResNet-18 (6-ch)};
\node[note, below=0.15cm of rnet] {\bfseries Frequency Stream};

% ---- Fusion ----
\node[sbox=yellow!15, right=1.5cm of pool, yshift=-1.3cm, minimum width=3cm, font=\small\bfseries] (fuse) {Gated Bilinear Fusion};

% ---- Heads ----
\node[hbox=red!10, right=1.2cm of fuse, yshift=0.55cm, minimum width=2cm] (ce) {Classification};
\node[hbox=red!10, right=1.2cm of fuse, yshift=0cm, minimum width=2cm] (sc) {SupCon};
\node[hbox=purple!10, right=1.2cm of fuse, yshift=-0.55cm, minimum width=2cm] (mk) {Mask Decoder};

% ---- Arrows: Input to streams ----
\draw[arr] (inp.north east) -- (enet.west) node[note, pos=0.5, above] {RGB};
\draw[arr] (inp.south east) -- (gray.west) node[note, pos=0.5, below] {};
...
% ---- Streams to fusion ----
\draw[arr] (pool.east) -- ++(0.4, 0) |- (fuse.175) node[note, pos=0.25, right] {$\vect{h}_{rgb}$\,\tiny(512-d)};
\draw[arr] (rnet.east) -- ++(0.4, 0) |- (fuse.185) node[note, pos=0.25, right] {$\vect{h}_{freq}$\,\tiny(512-d)};

% ---- Fusion to heads ----
\draw[arr] (fuse.east) -- ++(0.3, 0) |- (ce.west);
\draw[arr] (fuse.east) -- (sc.west);
\draw[arr] (fuse.east) -- ++(0.3, 0) |- (mk.west);



\end{tikzpicture}
\caption{DSAN architecture. Two parallel streams produce 512-d embeddings merged by gated fusion. The SRM and FFT outputs are concatenated into a 6-channel input before ResNet-18. The pre-pool spatial feature map from EfficientNet-B4 is also routed to the mask decoder (not shown for clarity). ``CPU'' and ``GPU'' labels indicate where each frequency operation runs.}
\label{fig:dsan_arch}
\end{figure*}
...
\node[note, above=0.06cm of k1] {SRM (CPU)};
...
\node[note, above=0.06cm of mag] {FFT (GPU)};
