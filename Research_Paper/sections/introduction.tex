...
\begin{enumerate}
    \item A dual-stream attribution architecture with gated bilinear fusion. We show formally that two-token self-attention degenerates into scalar interpolation, and that our gating mechanism provides roughly $64\times$ more per-dimension selectivity than standard $h{=}8$ multi-head attention (512 gate weights vs.\ 8 scalar interpolation weights).
